...
// See robust_state.spec.ts: clear the server-side persisted search so the no-search "idle" state
// is independent of prior tests/runs sharing the same SUT preset.
...
test('testUi', async ({ page }) => {
    ...
    await page.goto(`${BASE}/admin/users/suspended`);

    // UC04-S1: no search term -> idle (idle regardless of item count), with the no-items message shown.
    await rawPage.locator('.layout-tabular[data-state="idle"]').waitFor({ state: 'attached', timeout: stateTimeout });
    await rawPage.getByRole('heading', { name: 'No Users' }).waitFor({ state: 'visible', timeout: stateTimeout });

    await page.getByRole("button", { name: "search" }).click();
    await page.getByRole("searchbox", { name: "Search Items..." }).fill("zzzznomatch");

    // UC04-S4: an active search with no matches -> empty, with the no-results message shown.
    await rawPage.locator('.layout-tabular[data-state="empty"]').waitFor({ state: 'attached', timeout: stateTimeout });
    await rawPage.getByRole('heading', { name: 'No Results' }).waitFor({ state: 'visible', timeout: stateTimeout });
